...
router.post('/', async(req,res)=>{
    ...
    user = new User(_.pick(req.body, ['first_name','last_name','email','password','admin']));
    const salt = await bcrypt.genSalt(10);
    user.password = await bcrypt.hash(user.password, salt);
    await user.save();

    const token = user.generateAuthToken();
    return res.header('x-auth-token',token).send(_.pick(user,['_id','first_name','last_name','email','admin']));
});
